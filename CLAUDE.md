...
| TD-047 | Reported production bug: home dashboard stayed empty until pull-to-refresh or visiting Tareas/Pendientes. Hypothesis was that `MainScaffold._loadForHousehold` only warmed `TaskCubit.loadTimeline` (PDR-003's Todas tab) on startup and never `TaskCubit.load()` — the call that populates `buckets[TaskFilter.all]`, which `HomePage` actually reads via `TaskState.allTasks`. Audited against the code and git history: `_loadForHousehold` has always called both `load(householdId)` and `loadTimeline(householdId)` together, and `TaskCubit._upsert` (the shared engine behind create/update/complete/delete/applyRealtime) has always updated every `TaskFilter` bucket, including `all` — so this specific hypothesis does not hold for `HomePage`. The likely real cause of the reported symptom was the separate, already-fixed PDR-003 timeline staleness bug (commit `9284ae4`, landed the same day) and the null-`dueDate` 400 on task creation (commit `a2b320b`), both already on `main` before this audit branched off it. | Low | No source change needed for `HomePage`/`MainScaffold`. Added `frontend/test/widgets/home_page_test.dart` as regression coverage pinning down the already-correct behavior (existing task visible on start with no manual refresh; a locally created task appears via upsert) so a future regression here is caught by CI. | Won't fix (hypothesis refuted; no bug found) | TBD | 2026-08-15 |
