...
| `household:destruction_scheduled` | `{ householdId, scheduledAt, scheduledBy }` | The creator started the 24 h grace period (TD-067, PDR-022 D4). Household-wide: only the creator can schedule or cancel, but everyone living there is entitled to know their home is about to disappear |
| `household:destruction_cancelled` | `{ householdId }` | The creator called it off. Nothing had happened yet, so nothing is restored |
| `household:destroyed` | `{ householdId, destroyedAt, memberIds }` | The household is gone (TD-067, PDR-022 D4). Emitted after the commit, to the room and the member list captured BEFORE the memberships were deleted — the recipients cannot be recomputed afterwards because none of them is a member of anything any more. Clients purge their household-scoped cache and route away; every subsequent request answers 404 as the idempotent fallback |
...
| `household:savings_goal_created` | JointSavingsGoal | A joint savings goal was opened (TD-066 B10, PDR-018) |
| `household:savings_contribution` | `{ goalId, userId, amount, contributedCoins, targetCoins }` | Someone contributed. Household-wide because the per-member breakdown is explicitly public — UX-P1-SPEC §6 renders «Tú: 40 · Ana: 28» — unlike a wallet balance |
| `household:savings_goal_unlocked` | JointSavingsGoal | Contributions reached the item's price |
| `household:savings_goal_cancelled` | `{ goal, refunds[] }` | The goal was cancelled and every active contribution refunded |
...
**Server → Client events, personal room only:**
| Event | Payload | Trigger |
|-------|---------|---------|
...
| `economy:savings_refunded` | `{ goalId, amount }` | A cancelled goal returned this member's coins (TD-066 B10) |
...
| TD-067 | Gestión de roles y administración: promoción/degradación, transferencia, salida voluntaria, destrucción con período de gracia y protección contra hogares sin admin | High | **In progress (2026-09-01)** — gobernanza en PDR-022 |
...
| TD-071 | Reconocimiento entre miembros con cuatro reacciones cerradas a tareas completadas, notificación suave, actividad y contador semanal sin leaderboard | Medium | Open (2026-08-20; decisiones D1-D8 aprobadas, preguntas abiertas pendientes e implementación por completar; actividad depende de TD-070 y push real de TD-049) |
| TD-072 | Deep-link interno seguro desde una notificación FCM hasta la tarea exacta, con payload validado, autorización y fallback a Tareas | High | Open (2026-08-20; decisiones D1-D5 aprobadas, preguntas abiertas pendientes e implementación por completar; prueba push real depende de TD-049) |
