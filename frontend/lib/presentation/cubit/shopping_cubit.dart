...
class ShoppingCubit extends Cubit<ShoppingState> {
  ...
  Future<void> load(String householdId) async {
    ...
    try {
      final items = await _repo.list(householdId);
      emit(state.copyWith(status: ShoppingStatusUi.loaded, items: _sorted(items)));
    } on Failure catch (f) {
  ...
  void _upsert(ShoppingItem item) {
    ...
    emit(state.copyWith(status: ShoppingStatusUi.loaded, items: _sorted(list)));
  }
